...
import Models.Datacasa;
import Models.ResCasa;
import SingletonRequest.SingletonRequest;

public class RegistroCasa extends AppCompatActivity implements View.OnClickListener {
    String url = "http://54.90.119.130/api/casas/insertar";
    @Override
    ...
    public void onClick(View view) {
        ...
        JsonObjectRequest carta = new JsonObjectRequest(Request.Method.POST, url, jsonBody, new Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                Gson gson = new Gson();
                ResCasa casa;
                casa = gson.fromJson(response.toString(), ResCasa.class);
                SharedPreferences sharedPreferences = getSharedPreferences("casa", Context.MODE_PRIVATE);
                SharedPreferences.Editor editor = sharedPreferences.edit();
                editor.putString("idcasa", casa.getDatacasa().getId());
                editor.putString("nombrecasa", casa.getDatacasa().getNombre_casa());
                editor.apply();
